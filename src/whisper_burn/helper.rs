use burn::{
    prelude::ToElement,
    tensor::{BasicOps, Element, Numeric, Tensor, TensorKind, activation::relu, backend::Backend},
};
...
pub fn _tensor_min_scalar<B: Backend, const D: usize>(x: Tensor<B, D>, min: f64) -> Tensor<B, D> {
...
pub fn all_zeros<B: Backend, const D: usize>(x: Tensor<B, D>) -> bool {
    x.abs().max().into_scalar().to_f64() == 0.0
}
