...
impl Gpt2Tokenizer {
    ...
    pub fn _encode(&self, text: &str) -> Vec<usize> {
...
impl ToString for SpecialToken {
    fn to_string(&self) -> String {
        match self {
            ...
            SpecialToken::_Translate => "<|translate|>".into(),
            SpecialToken::Transcribe => "<|transcribe|>".into(),
            SpecialToken::_StartofLM => "<|startoflm|>".into(),
            SpecialToken::StartofPrev => "<|startofprev|>".into(),
            SpecialToken::_NoSpeech => "<|nospeech|>".into(),
...
fn _construct_special_tokens() -> Vec<AddedToken> {
    ...
    let lang_keys = _LANGUAGES.iter().map(|lang| format!("<|{}|>", lang));
